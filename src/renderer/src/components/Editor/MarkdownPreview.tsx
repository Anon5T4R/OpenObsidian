...
export default function MarkdownPreview({ content, onWikiLinkClick, onChange, vaultPath }: MarkdownPreviewProps) {
  const html = useMemo(() => {
    ...
    // Only applies when vaultPath is an absolute filesystem path (desktop).
    // Android internal paths ("VaultName") and SAF paths ("content://...") are
    // intentionally excluded because file:// won't be served by the WebView there.
    const isAbsPath = (p: string) => p.startsWith('/') || /^[a-zA-Z]:[/\\]/.test(p)
    if (vaultPath && isAbsPath(vaultPath)) {
